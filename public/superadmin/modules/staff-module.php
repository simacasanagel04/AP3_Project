...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add'])) {
        ...
        if (is_numeric($staffId) && $staffId > 0) {
            // --- SUCCESS PATH (Staff record created) ---
            ...
            if (!empty($submittedEmail)) { 
                ...
                if ($userCreationResult === true || (is_string($userCreationResult) && strpos($userCreationResult, '✅') !== false)) {
                    ...
                    // User account creation FAILURE (Staff record exists, User record failed)
                    
                    $customErrorMessage = $userCreationResult;
                    
                    // Check if the error indicates a duplicate email conflict
                    // We assume the User class returns a string containing 'exists' or 'duplicate' on conflict.
                    if (is_string($userCreationResult) && (strpos($userCreationResult, 'exists') !== false || strpos($userCreationResult, 'duplicate') !== false || strpos($userCreationResult, 'Email already') !== false)) {
                        $customErrorMessage = "Failed to create account, email or contact number may already exist.";
                    }

                    $message = "⚠️ Staff record ID {$staffId} was saved, but failed to create user account. Error: " . htmlspecialchars($customErrorMessage);
                }
            } else {
                $userMessage = "⚠️ Staff added successfully, but no email provided for user creation.";
            }

        } else {
            // --- FAILURE PATH (Staff creation failed) ---
            $message = "❌ " . $staffId;
        }
    }
...
if (!empty($search)) {
    ...
    // MODIFICATION START: Sort records by staff_id ascending to display from ID 1
    if (is_array($records) && !empty($records)) {
        usort($records, function($a, $b) {
            // Use the spaceship operator for concise comparison (a <=> b for ascending)
...
<?php if ($message): ?>
<div class="alert <?= strpos($message, '❌') !== false || strpos($message, '⚠️') !== false ? 'alert-danger' : 'alert-info' ?>"><?= $message ?></div>
<?php endif; ?>
<?php if ($userMessage): ?>
...
<div id="addFormStaff" class="collapse mb-4">
    <div class="card card-body shadow-sm">
        <form method="POST" class="row g-3">
            ...
            <button name="add" class="btn btn-primary">Save Staff & Create User Account</button>
                ...
                <tr>
                    <form method="POST">
                        ...
                        <td>
                            ...
                            <button name="delete" value="<?= $r['staff_id'] ?? '' ?>" class="btn btn-sm btn-danger w-100" onclick="return confirm('⚠️ Delete this staff member and their user account?')">Delete</button>
                        </td>
                    </form>
                </tr>
...
function copyPassword(buttonElement) {
    ...
    // Non-blocking message replacement (Replaced alert())
