...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (is_numeric($result) && $result > 0) {
            ...
            if (!empty($submittedEmail)) { 
                ...
                elseif(is_string($userCreationResult) && (strpos($userCreationResult, 'exists') !== false || strpos($userCreationResult, 'duplicate') !== false || strpos($userCreationResult, 'Email already') !== false)) {
                    $message = "⚠️ Doctor record ID {$displayDoctorId} was saved, but email already exists in the system.";
                    $autoGeneratedPassword = '';
                }
                else {
    ...
    elseif (isset($_POST['update'])) {
        ...
        // FIX: Use absolute path for redirect
        $currentScript = $_SERVER['SCRIPT_NAME']; // Gets /public/superadmin/superadmin_dashboard.php
        header("Location: " . $currentScript . "?module=doctor");
        exit();
    }
