...
<div class="d-flex justify-content-between align-items-center mb-4 p-3 bg-white rounded-3 shadow-sm border">
  <form class="d-flex w-50" method="GET">
    ...
    <?php endforeach; ?>
    <input class="form-control me-2 rounded-pill border-primary" type="search" name="search_appt" placeholder="Search by Appointment ID..." value="<?= htmlspecialchars($search_term) ?>">
  ...
  <?php if ($is_superadmin || $is_staff): ?>
    <button class="btn btn-success" data-bs-toggle="collapse" data-bs-target="#addFormAppt">Schedule New Appointment</button>
  <?php endif; ?>
</div>
  ...
  <div class="card card-body shadow-sm border rounded bg-light">
    <form method="POST" class="row g-3">
      ...
      <div class="col-md-3">
        ...
        <input type="date" name="APPT_DATE" required class="form-control" value="<?= date('Y-m-d') ?>" min="<?= date('Y-m-d') ?>">
      </div>
      <div class="col-md-3">
        <label class="form-label fw-semibold">Time *</label>
        <input type="time" name="APPT_TIME" required class="form-control">
      </div>
      <div class="col-md-6">
      ...
      <div class="col-md-4">
        ...
        <select name="STAT_ID" required class="form-select">
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <p>Delete Appointment ID: <strong id="modalApptIdDisplay"></strong>?</p>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.querySelectorAll('[data-bs-target="#deleteApptModal"]').forEach(btn => {
    btn.addEventListener('click', function() {
      ...
      document.getElementById('modalApptIdDisplay').textContent = id;
      document.getElementById('deleteApptIdInput').value = id;
    });
  });
});
</script>
